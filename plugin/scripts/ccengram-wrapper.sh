#!/bin/bash
# CCEngram wrapper - ensures binary exists then runs it
...
download_binary() {
    ...
    echo "CCEngram: Downloading ${version} for ${platform}..." >&2
    ...
    echo "CCEngram: Installed ${version}" >&2
}
...
ensure_binary() {
    ...
    if [ -z "$latest_version" ]; then
        echo "CCEngram Error: Binary not found at $BINARY" >&2
